...
class IFSuperResolutionPipeline(DiffusionPipeline, LoraLoaderMixin):
    ...
    def __call__(
        ...
        num_channels = self.unet.config.in_channels // 2 # 6임
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                #print(self.scheduler.config.variance_type)
                # compute the previous noisy sample x_t -> x_t-1
                intermediate_images = self.scheduler.step(
                    noise_pred, t, intermediate_images, **extra_step_kwargs, return_dict=False
                )[0]
                #print(f'noised {noise_pred.shape}')
                #print(f'intermediate {intermediate_images.shape}')
